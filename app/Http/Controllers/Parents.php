...
class Parents extends CI_Controller {
    ...
    function attendance_report_selector()
    {
        ...
        redirect(base_url().'index.php?parents/student_profile/student_attendance_report/' . $student_code . '/' . $month . '/' . $year, 'refresh');
    }
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'update_profile_info')
        {
            ...
            if($_FILES['image']['name'] != '')
                //move_uploaded_file($_FILES['image']['tmp_name'], 'uploads/parent_image/' . $this->session->userdata('login_user_id') . '.jpg');
        ...
        if ($param1 == 'change_password')
        {
            ...
            $current_password_db = $this->db->get_where('parent', array('parent_id' => $this->session->userdata('login_user_id')))->row()->password;
    ...
    {
        $parents_id = $this->db->get_where('class', array('class_id' => $class_id))->row()->parents_id;
        $parentss   = $this->db->get_where('parents', array('parents_id !=' => $parents_id))->result_array();

        foreach ($parentss as $row) {
            echo '<option value="' . $row['parents_id'] . '">' . $row['name'] . '</option>';
        }
    }

    function get_class_tests($class_id = '', $class_group_id = '', $course_id = '')
    {
        $parents_id     = $this->session->userdata('parents_id');
        $class_tests    = $this->db->get_where('class_test', array('parents_id' => $parents_id, 'class_id' => $class_id, 'class_group_id' => $class_group_id, 'course_id' => $course_id))->result_array();
